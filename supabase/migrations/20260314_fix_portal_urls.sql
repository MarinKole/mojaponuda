...
-- New portal is at next.ejn.gov.ba/procedures/{id}/overview

-- Step 1: Fix old format (www.ejn.gov.ba/Notice/{id})
UPDATE tenders
SET portal_url = 'https://next.ejn.gov.ba/procedures/' || 
...
-- Step 3: Fix /bs/ prefix format if already applied
UPDATE tenders
SET portal_url = REPLACE(portal_url, '/bs/procedures/', '/procedures/')
WHERE portal_url LIKE 'https://next.ejn.gov.ba/bs/procedures/%';
